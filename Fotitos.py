...
def recognize_digit(image, segments_coords, threshold=85):
...
print("\nPrimer dígito BPM (solo segmento g):")
g_segment_coords = (455,254,477,264)
is_on, avg_val = check_segment(gray, g_segment_coords, THRESHOLD)
print(f"Segmento g: {is_on} - {avg_val:.1f}")
bpm += '1' if is_on else '0'
print(f"Dígito reconocido: {'1' if is_on else '0'}")
print("-" * 40)

# Resto de dígitos BPM (ahora dígitos 2 y 3)
for i, digit_segments in enumerate(bpm_coords, 2):
    print(f"\nDígito {i}:")
    digit = recognize_digit(gray, digit_segments, THRESHOLD)
    bpm += digit
